...
from utils import sdbm_hash, async_func, async_cor_func
...
def do_Join():
    ...
    if not name:
        ...
        tasks = async_cor_func("[A]"), async_cor_func("[B]")
        ...
        tasks = [
            ...
            # asyncio.ensure_future(keepalive(msg_check_mem, roomchat_sock, username)),

            asyncio.ensure_future(async_func("[async_func]")),
            asyncio.ensure_future(async_func("[async_func2]")),

            # # Step 4. start my TCP server, as the server for other users to CONNECT to in the chatroom
            # asyncio.ensure_future(build_tcp_server(msg_check_mem)),
            # # Step 5. start a TCP client, to CONNECT another user's TCP server in the chatroom
            # asyncio.ensure_future(retain_forward_link(msg_check_mem, myHashID)),
        ]
...
def retain_forward_link(msg_check_mem, myHashID):
    global my_tcp_conns, roomname, username, myip, myport, MsgWin, \
        roomchat_sock, sock_peers, forward_link, msgID
